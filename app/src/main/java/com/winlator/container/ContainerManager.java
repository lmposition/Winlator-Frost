...
public class ContainerManager {
    ...
    public void exportShortcutsForDaijisho() {
        File exportDir = new File(Environment.getExternalStorageDirectory(), "WinlatorShortcuts");
        if (!exportDir.exists()) {
            exportDir.mkdirs();
        }

        ArrayList<Shortcut> shortcuts = loadShortcuts();
        
        for (Shortcut shortcut : shortcuts) {
            String fileName = shortcut.name.replaceAll("[^a-zA-Z0-9._-]", "_") + ".sh";
            File launcherFile = new File(exportDir, fileName);
            
            String launcherContent = createDaijishoLauncher(shortcut);
            FileUtils.writeString(launcherFile, launcherContent);
            
            if (launcherFile.exists()) {
                FileUtils.chmod(launcherFile, 0755);
            }
        }
    }

    private String createDaijishoLauncher(Shortcut shortcut) {
        StringBuilder content = new StringBuilder();
        content.append("#!/system/bin/sh\n");
        content.append("# Winlator Shortcut Launcher for ").append(shortcut.name).append("\n");
        content.append("# Container: ").append(shortcut.container.getName()).append("\n");
        content.append("# Generated by Winlator-Frost\n\n");
        
        content.append("am start -n com.winlator/.XServerDisplayActivity ");
        content.append("--ei container_id ").append(shortcut.container.id).append(" ");
        content.append("--es shortcut_path \"").append(shortcut.file.getPath()).append("\"\n");
        
        return content.toString();
    }

    public void exportShortcutsForDaijishoAsync(Runnable callback) {
        final Handler handler = new Handler();
        Executors.newSingleThreadExecutor().execute(() -> {
            exportShortcutsForDaijisho();
            handler.post(callback);
        });
    }
}
